...
function _page($$payload, $$props) {
  ...
  if (image != null) {
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$payload.out += `<div class="group relative"><img class="w-full max-w-full object-cover"${attr("src", item.url)}${attr("alt", item.url)}> <button class="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 m-auto transition-all">Delete</button></div>`;
    }
    $$payload.out += `<!--]-->`;
  } else {
    $$payload.out += "<!--[!-->";
